...
def toeplitz(seq: Tensor, mode: str) -> Tensor:
    """
    Construct Toeplitz-matrix by shifting 1d sequence discarding even rows.

    Parameters
    ----------
    seq: float-valued PyTorch tensor of size [batch_size m]
        batch of sequences (row vectors) defining 1d-Toeplitz matrices
    mode: str in {"left", "right"}
        determines whether to shift sequence to the left or right

    Returns
    -------
    toeplitz_mat: float-valued PyTorch tensor of size [batch_size m 2m - 1]
        Toeplitz matrix obtained by shifting sequence discarding even rows.
    """
    m = compute_order(seq.shape[1:]).item()
    num_filters = seq.shape[0]

    # Construct row and columnn indices associated to right or left shifts in steps of two.

    # Row indices [0 ... 0 ... ... m -1 ... m -1]
    rows = torch.arange(m).view(m, 1).repeat(1, 2 * m - 1).flatten()

    if mode == "right":
        # Column indices [0 ... 2(m-1) ... ... 2(m-1) ... 4(m-1)]
        cols = torch.arange(2 * m - 1).view(1, 2 * m - 1).repeat(m, 1) + torch.arange(
            0, 2 * m, 2
        ).view(m, 1)
    elif mode == "left":
        # Column indices [2(m-1) ... 4(m-1) ... ... 0 ... 2(m-1)]
        cols = torch.arange(2 * (m - 1), 4 * (m - 1) + 1).view(1, 2 * m - 1).repeat(
            m, 1
        ) - torch.arange(0, 2 * m, 2).view(m, 1)
    else:
        raise NotImplementedError("Prescribed mode should be either left or right")
    cols = cols.flatten()

    # Repeat (row, col) indices for each sequence in batch
    num_elements = m * (2 * m - 1)
    cols = cols.repeat(num_filters)
    rows = rows.repeat(num_filters)
    batch = (
        torch.arange(num_filters).view(num_filters, 1).repeat(1, num_elements).flatten()
    )

    # Construct Toeplitz matrix
    mat = torch.zeros(num_filters, m, 4 * m - 3).to(seq.device)
    if mode == "right":
        mat[batch, rows, cols] = seq.repeat(1, m).flatten()
        return mat[..., 0 : 2 * m - 1]
    elif mode == "left":
        mat[batch, rows, cols] = seq.repeat(1, m).flatten()
        return mat[..., 2 * (m - 1) :]
